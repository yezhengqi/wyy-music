<template>
  <div class="play">
    ...
    <audio
      ref="audio"
      preload="true"
      :src="`https://music.163.com/song/media/outer/url?id=${this.id}.mp3`"
    ></audio>
  </div>
</template>
...
export default {
  ...
  methods: {
    async getSong () { // 获取歌曲详情, 和歌词方法
      const res = await this.$API.play.getSongById(this.id)

      this.songInfo = res.songs[0]
      // 获取-并调用_formatLyr方法, 处理歌词
      const lyrContent = await this.$API.play.getLyricById(this.id)
      const lyricStr = lyrContent.lrc.lyric
